...
if [ -d "results" ] && [ "$(ls -A results)" ]; then
  ...
  echo -e "${CYAN}Report for scenario: $SCENARIO_SUFFIX${NC}"
  
  # Build and run the report generation container
  docker build -t report-generator -f dockerfile.report .
  docker run --rm \
    -v "$(pwd)/aggregated-report:/app/aggregated-report" \
    -e SCENARIO_SUFFIX="$SCENARIO_SUFFIX" \
    report-generator "$SCENARIO_SUFFIX"

  # Clean up
  docker rmi report-generator

  if [ -f "aggregated-report/index.html" ]; then
