    ...
    def export_html_report(self, output_dir="./aggregate-report"):
        ...
        try:
            if BS4_AVAILABLE:
                ...
                print(f"[reporters.py] sample_scenario: {self.aggregator.sample_scenario}")
                scenario_short_name = self.aggregator.sample_scenario.split('.')[-1] if self.aggregator.sample_scenario else "Unknown"
                print(f"[reporters.py] scenario_short_name: {scenario_short_name}")
                ...
                # Update the date in Run Information section
                run_info_title = soup.find('span', class_='simulation-information-title', string='Run Information')
                if run_info_title:
                    container = run_info_title.find_next('div', class_='simulation-information-container')
                    if container:
                        # Update Date
                        date_item = container.find('span', class_='simulation-information-item')
                        if date_item:
                            date_label = date_item.find('span', class_='simulation-information-label', string='Date: ')
                            if date_label:
                                # Get current time in GMT
                                current_time = datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S GMT')
                                date_label.find_next('span').string = current_time
                        
                        # Update Duration
                        duration_item = container.find('span', class_='simulation-information-item')
                        if duration_item:
                            duration_label = duration_item.find('span', class_='simulation-information-label', string='Duration: ')
                            if duration_label:
                                duration_label.find_next('span').string = '—'
